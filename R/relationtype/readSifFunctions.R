...
makeSingleEdge <- function (inFile, outFile, edgeTypes) {
    inFileList <- readFile(inFile)
    connections <- combineConnections(inFileList, edgeTypes)
...
readFile <- function (inFile) {
    ...
    result = tryCatch({
        while ( TRUE ) {
            ...
            theNode <- theLine[1]
            typeConn <- theLine[2]
            fileOutputList[[typeConn]][[theNode]] <- list()
...
combineConnections <- function (inFileList, connTypes = NULL) {
    connections <- list()
...
#' generated by \code{combineConnections}.
#'
...
writeFile <- function (outFile, connections) {
    ...
    proteinNames <- unique(c(unlist(connections),
                             names(connections)))
